...
describe('BasicAllowance', () => {
  ...
  it('amino type', () => {
    const ba = new BasicAllowance('1000ujmes', now);
  ...
  it('both set', () => {
    ...
    expect(ba.toData().spend_limit).toEqual(new Coins('1000ujmes').toData());
    expect(ba.toData().expiration).toEqual(now.toISOString());
    expect(ba.toProto().spendLimit).toEqual(new Coins('1000ujmes').toProto());
    expect(ba.toProto().expiration).toEqual(now);
    expect(ba.toAmino().value.spend_limit).toEqual(
      new Coins('1000ujmes').toAmino()
    );
    expect(ba.toAmino().value.expiration).toEqual(now.toISOString());
  });

  it('spend_limit only', () => {
    const ba = new BasicAllowance('1000ujmes', undefined);
  ...
  it('spend_limit has zero amount', () => {
    expect(() => new BasicAllowance('1ujmes,-1uusd', undefined)).toThrowError();
